...
def calculate_quality_contact_stats(player_df):
    ...
    batted_balls = player_df.dropna(subset=["ExitVelocity", "HH"], how='all')
    
    ev = batted_balls["ExitVelocity"].dropna() if "ExitVelocity" in batted_balls else pd.Series(dtype=float)
    ...
    air_ev = batted_balls.loc[batted_balls["BBT"].isin(["LD", "OFFB", "IFFB"]), "ExitVelocity"].dropna()
    stats["Air EV"] = f"{air_ev.mean():.1f}" if not air_ev.empty else "0.0"
    
    # Barrel%
    batted_balls["is_barrel"] = batted_balls.apply(
        lambda row: is_barrel(row["ExitVelocity"], row["LaunchAngle"]) if pd.notna(row["ExitVelocity"]) else False,
        axis=1
    )
    ...
    def is_hard_hit(row):
        if pd.notna(row["ExitVelocity"]):
            return row["ExitVelocity"] >= 92
        elif pd.notna(row["HH"]):
            return row["HH"] == 1
        else:
            return False
    
    stats["Hard-Hit%"] = round(batted_balls.apply(is_hard_hit, axis=1).mean() * 100, 1) if not batted_balls.empty else 0
...
def calculate_plate_metrics(player_df):
    ...
    total_pa = player_df["AB Result"].notna().sum()
    ...
    valid_coords = player_df["Strike Zone Side"].notna() & player_df["Strike Zone Height"].notna()
    missing_coords = ~valid_coords
    
    z_mask = player_df["PitchZone"].isin(in_zone) & valid_coords
    o_mask = valid_coords & ~z_mask
    
    z_swings = (swings_mask & z_mask).sum()
    o_swings = (swings_mask & o_mask).sum()
    
    z_contacts = (contacts_mask & z_mask).sum()
    o_contacts = (contacts_mask & o_mask).sum()
    
    if "Chase" in player_df.columns:
        o_chases = (swings_mask & o_mask).sum()
        
        o_balls = 0
        if missing_coords.any():
            missing_df = player_df[missing_coords]
            chases_missing = (missing_df["Chase"] == 1)
            balls_missing = missing_df["Ball/Strike"].eq("Ball")
            o_chases += chases_missing.sum()
            o_balls += balls_missing.sum()
        
        chase_denominator = o_chases + o_balls
        stats["Chase%"] = round((o_chases / chase_denominator) * 100, 1) if chase_denominator > 0 else 0
    else:
        stats["Chase%"] = 0

    
    stats["Z-Contact%"] = round(z_contacts / z_swings * 100, 1) if z_swings > 0 else 0
    stats["O-Contact%"] = round(o_contacts / o_swings * 100, 1) if o_swings > 0 else 0
    stats["Z-Swing%"] = round(z_swings / z_mask.sum() * 100, 1) if z_mask.sum() > 0 else 0
...
def calculate_pitcher_advanced_stats(player_df, fip_constant=4.853):
    ...
    AB = player_df[~player_df["AB Result"].isin(["BB", "HBP", "SAC"]) & player_df["AB Result"].notna()].shape[0]
    ...
    if player_type == "Pitcher" and page == "Pitch Usage":
        ...
        def calculate_qoc_allowed(player_df):
            ...
            def is_hard_hit(row):
                if pd.notna(row["ExitVelocity"]):
                    return row["ExitVelocity"] >= 92
                elif "HH" in row and pd.notna(row["HH"]):
                    return row["HH"] == 1
                else:
                    return False

            stats["Hard-Hit%"] = round(batted_df.apply(is_hard_hit, axis=1).mean() * 100, 1) if not batted_df.empty else 0
        ...
        def calculate_pitching_effectiveness(df):
            ...
            valid_zone = df['Strike Zone Side'].notna() & df['Strike Zone Height'].notna()

            in_zone = valid_zone & (df['Strike Zone Side'].between(-8.9, 8.9)) & \
                      (df['Strike Zone Height'].between(17.5, 42.5))

            strikes = df['Ball/Strike'].apply(lambda x: 1 if x == "Strike" else 0).sum()
            zone_pitches = in_zone.sum()
            o_pitches = (valid_zone & ~in_zone).sum()
            swings_in_zone = df[in_zone & df['Pitch Result'].isin(["Miss", "Contact"])].shape[0]
            swings_out_zone = df[valid_zone & ~in_zone & df['Pitch Result'].isin(["Miss", "Contact"])].shape[0]

            z_whiff = df[in_zone & (df['Pitch Result'] == "Miss")].shape[0]
            o_whiff = df[valid_zone & ~in_zone & (df['Pitch Result'] == "Miss")].shape[0]

            total_swings = df['Pitch Result'].isin(["Miss", "Contact"]).sum()

            total_whiff = df['Pitch Result'].eq("Miss").sum()

            chase = (swings_out_zone / o_pitches * 100) if o_pitches > 0 else 0
            csw = ((called_strikes.sum() + total_whiff) / total_pitches) * 100

            pa_counts = df["AB Result"].notna().sum()
            ...
            stats["Whiff%"] = round((total_whiff / total_swings) * 100, 1) if total_swings > 0 else 0
